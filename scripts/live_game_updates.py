  ...
  for i, game in enumerate(games):
      ...
      for j, player in enumerate(player_stats):
        ...
        for stat_name in stat_name_list:
          update_prop(stat_name["db_name"], str(player["personId"]), str(game["gameId"]), stats[stat_name["api_name"]])
          total_updated += 1
        ...
        update_prop("pts_ast", str(player["personId"]), str(game['gameId']), pts_ast)
        total_updated += 1
          
        reb_ast = stats["reboundsTotal"] + stats["assists"]
        update_prop("reb_ast", str(player["personId"]), str(game['gameId']), reb_ast)
        total_updated += 1
