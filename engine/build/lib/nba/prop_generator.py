...
from shared.prop_generation.base import GameData, PropConfig
...
class NbaPropGenerator(BasePropGenerator[NbaPlayerStats, NbaGames]):
    ...
    def generate_prop_for_stat(
        self, stat_name: str, game_data: GameData[NbaPlayerStats, NbaGames]
    ) -> float:
    ...
    def _generate_special_percentage_prop(
        self, config: PropConfig, game_data: GameData[NbaPlayerStats, NbaGames]
    ) -> float:
        ...
        if config.stat_name == "blk":
            ...
            for i, player_game in enumerate(game_data.player_games):
                team_blk = (
                    game_data.team_games[i].blk if i < len(game_data.team_games) else 1
                )
            ...
            for i, player_game in enumerate(game_data.player_games):
                team_stl = (
                    game_data.team_games[i].stl if i < len(game_data.team_games) else 1
                )
        ...
        for feature in config.features:
            if feature.name in ["pct_blk_a", "pct_stl_a"]:
                ...
                prediction_data[feature.name] = (
                    self.feature_extractor.extract_prediction_feature_value(
