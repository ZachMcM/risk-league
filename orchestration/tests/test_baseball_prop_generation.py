...
def main() -> None:
    """Test script to generate MLB props without DB insertion - outputs to file."""
    try:
        # Parse command line arguments
        if len(sys.argv) < 2:
            print("Usage: python test_baseball_prop_generation.py <output_file> [date]")
        ...
        for i, game in enumerate(games_list):
            logger.info(f"Processing MLB game {game['game_ID']} ({i + 1}/{len(games_list)})")
